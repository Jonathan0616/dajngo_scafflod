...
SECRET_KEY = '(sfy*__h*r(-e85gjo2j(4&8zkvgafvpd979193^$i_2f7f@4*'
...
LOGGING = {
    ...
    'handlers': {
        ...
        'info_file': {
            ...
            'filename': os.path.join(LOG_DIR, 'info-%s.log' % (time.strftime('%Y-%m-%d'))),
        ...
        'error_file': {
            'class': 'logging.handlers.RotatingFileHandler',
            'filename': os.path.join(LOG_DIR, 'error-%s.log' % (time.strftime('%Y-%m-%d'))),
            'maxBytes': 5 * 1024 * 1024,
            'backupCount': 10,
            'encoding': 'utf8',
            'level': 'ERROR',
