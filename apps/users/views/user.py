...
class UsersView(APIView):
    def get(self, request: Request, *args, **kwargs) -> JsonResponse:
        ...
        queryset: QuerySet = User.objects.filter(is_deleted=0). \
            only('user_id', 'name', 'user_type', 'create_time').order_by('-create_time')
        # 分页
        ...
        for obj in queryset:
            ...
            elem.createTime = obj.create_time
            data.append(elem)
...
class UserView(APIView):
    ...
    def patch(self, request: Request, user_id: str, *args, **kwargs):
        name = request.data.get('name')
        now = datetime.now()
        if User.objects.exclude(user_id=user_id).filter(is_deleted=0, name=name).exits():
            raise ECException(ECEnum.UserExist)
