...
class IfNull(Func, ABC):
    ...
    def as_mysql(self, compiler, connection, **extra_context):
        ...
        return super(IfNull, self).as_mysql(
            compiler, connection, default_value=default_value,
            template="%(function)s(%(expression)s, %(default_value)s)",
            **extra_context
        )
...
class ConcatWS(Func, ABC):
    ...
    def as_mysql(self, compiler, connection, **extra_context):
        # user: CONCAT_WS(';', 'ugc_type', 'tile_name')
        sep = "'%s'" % self.sep
        return super().as_mysql(
...
class JsonExtract(Func, ABC):
    ...
    def __int__(self, json_field, path, output_field=None, **extra):
        """
        :param json_field: 字段名
        :param path: Union[int, str]
                int：位置 0起始
                str: 关键字
        """
        super(JsonExtract, self).__int__(output_field=output_field, **extra)
